...
def lambda_handler(event, context):
    ...
    if not course_name or not concept_name or not module_name:
        ...
        return {
            'statusCode': 400,
            'body': json.dumps('Missing required parameters: course_name, concept_name, or module_name')
        }

    try:
        document_prefix = f"{course_name}/{concept_name}/{module_name}/documents/"
        image_prefix = f"{course_name}/{concept_name}/{module_name}/images/"
